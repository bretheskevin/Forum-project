export function likeAndDislike() {
    ...
    function makeLikeGrey(like) {
        ...
        like.children[1].classList.remove("txt-green");
    }

    function makeDislikeRed(dislike) {
    ...
    function makeDislikeGrey(dislike) {
    ...
    for (let post of posts) {
        const footer = post.children[2];
        const likes = footer.children[0];
        const dislikes = footer.children[1];
        const likesSvgContent = likes.children[0];
        const dislikesSvgContent = dislikes.children[0];

        likes.addEventListener("click", () => {
            likesSvgContent.innerHTML = likeSvgGreen;
            makeLikeGreen(likes);
            makeDislikeGrey(dislikes);
        })

        dislikes.addEventListener("click", () => {
            dislikesSvgContent.innerHTML = dislikeSvgRed;
            makeDislikeRed(dislikes);
            makeLikeGrey(likes)
        })
    }
}
